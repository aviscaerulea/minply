...
9. WASAPI セッション開始＆再生（PlayAudio）
   ├─ リードイン 1.5 秒（19kHz ガードトーン）
   ├─ オーディオデータ再生
   └─ リードアウト 2.0 秒（19kHz ガードトーン）
10. バッファドレイン待機
11. リソース解放＆終了
```
...
```cpp
constexpr float LEAD_IN_DURATION = 1.5f;    // リードイン時間（秒）：BLE 起床 0.7 秒 + WASAPI セッション起動ノイズ吸収マージン
constexpr float LEAD_OUT_DURATION = 2.0f;   // リードアウト時間（秒）：リードインより長い。音声末尾が BLE/SBC コーデック遅延パイプラインを通過するまで待機するため
...
1. **単一 WASAPI セッションでの 19kHz ガードトーン再生**
   ...
   - リードイン（1.5 秒）: BLE レシーバを起床し、WASAPI セッション起動ノイズを吸収
   - 本編オーディオ: デジタルサイレンスなしで連続再生
   - リードアウト（2.0 秒）: 本編末尾での BLE スリープを防止
